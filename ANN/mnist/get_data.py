...
if __name__ == "__main__":
    ...
    imgs = imgs / 255.0
    np.savetxt('train-images.csv', imgs, fmt='%10.3f', delimiter=',')

    imgs, data_head = loadImageSet(file3)
    imgs = imgs / 255.0
    np.savetxt('test-images.csv', imgs, fmt='%10.3f', delimiter=',')

    labels, labels_head = loadLabelSet(file2)
    np.savetxt('train-labels.csv', labels, fmt='%d', delimiter=',')

    labels, labels_head = loadLabelSet(file4)
    np.savetxt('test-labels.csv', labels, fmt='%d', delimiter=',')
